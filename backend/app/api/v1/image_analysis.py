...
@router.post(
    "/cross-index",
    response_model=VisualElementCorrelationResponse,
    summary="Cross-index visual elements with campaign performance analytics",
)
async def cross_index_visual_elements_with_analytics(
    payload: VisualElementCorrelationRequest,
) -> VisualElementCorrelationResponse:
    """
    Cross-index visual elements from image analysis with campaign performance metrics.
    This combines image analysis insights with analytics data to identify the most impactful visual elements.
    """
    try:
        # Get correlation analysis from image analysis service
        result = image_analysis_service.correlate_visual_elements_with_performance(
            visual_elements=payload.visual_elements,
            date_range=payload.date_range,
            min_campaigns=payload.min_campaigns,
        ...
        # 1. Query analytics service for campaign performance metrics
        # 2. Match campaigns with analyzed images
        # 3. Correlate visual elements with performance data
        # 4. Generate insights based on actual data
        ...
        return VisualElementCorrelationResponse(
            correlations=correlations,
            summary=result.get("summary", "Cross-indexing analysis completed"),
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Cross-indexing failed: {str(e)}")
